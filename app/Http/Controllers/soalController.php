...
class soalController extends Controller
{
   ...
   public function destroy($id)
    ...
    public function kick($bidang, $data)
    {

       DB::table('tb_pembayaran_tf')->where('id', '=', $data)->where('id_bidang', '=', $bidang)->delete();
        alert()->success('Berhasil.','Data telah dihapus!');
        return Redirect::back();
    }

    public function kick2($bidang, $data)
    {

       DB::table('tb_pembayaran_ss')->where('id', '=', $data)->where('id_bidang', '=', $bidang)->delete();
        alert()->success('Berhasil.','Data telah dihapus!');
        return Redirect::back();
    }
    ...
    // } 
        ...
        $siswa = DB::table('tb_pembayaran_tf')->where('tb_pembayaran_tf.id_bidang', '=', $id)->join('users','tb_pembayaran_tf.id','=','users.id')->get();

        $siswaSS = DB::table('tb_pembayaran_ss')->where('tb_pembayaran_ss.id_bidang', '=', $id)->join('users','tb_pembayaran_ss.id','=','users.id')->get();

        $idUser = Auth::user()->id;
        $userLogin = DB::table('users')->where('id',$idUser)->first();

        return view('soal.pilih-tryout',compact('data','dat','datat','dataf', 'siswa', 'userLogin', 'siswaSS'));
    } 
