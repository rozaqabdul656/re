                ...
                <div class="card shadow mb-4">
                  ...
                  <div class="card-body">
                    <div class="row" style="padding: 20px;">
                      ...
                      >
                        <div class="card">
                          <h5 class="card-title paket"> Daftar Siswa Bidang {{$da->bidang}}</h5>
                            <div class="card-body">
                              <ul class="detail">
                                  @php $no=0 @endphp
                                  <table class="table">
                                    ...
                                    <tr>
                                      ...
                                      <td>
                                        <a onclick="return confirm('Keluarkan peserta dari kelas ini ?')" href="{{url('kick-peserta',['bidang' => $s->id_bidang,'data' => $s->id] )}}"><i class="fa fa-trash"></i></a>
                                      </td>
                                    </tr>
                                    @endforeach
                                    @foreach($siswaSS as $ss)
                                    @php $no++ @endphp
                                    <tr>
                                      <td>{{ $no }}</td>
                                      <td>{{ $ss->name }}</td>
                                      <td>{{ $ss->alamat }}</td>
                                      <td>{{ $ss->asal_sekolah }}</td>
                                      <td>
                                        <a onclick="return confirm('Keluarkan peserta dari kelas ini ?')" href="{{url('kick-peserta2',['bidang' => $ss->id_bidang,'data' => $ss->id] )}}"><i class="fa fa-trash"></i></a>
                                      </td>
                                    </tr>
                                    @endforeach
                                  </table>
